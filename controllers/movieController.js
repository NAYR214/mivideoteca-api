...
exports.createMovie = async (req, res) => {
  const { title, director, year, posterUrl, rating, isFavorite } = req.body;
...
exports.toggleFavorite = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Error cambiando favorito' });
  }
};
...
exports.setRating = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Error actualizando rating' });
  }
};